...
const TaskList: React.FC<TaskListState> = ({ status }) => {
  ...
  const [tasks, setTasks] = useState<Paged<TaskInfo[]>>({
    total: 0,
    list: [],
  });
  const [inProgress, trackedGetCreatedTasks] = usePromiseTrack(getCreatedTasks);

  const loadTasks = () => {
    if (status !== null) {
      const pagination = { page, size: SIZE };

      trackedGetCreatedTasks(status, pagination).then(response => {
        setTasks({
